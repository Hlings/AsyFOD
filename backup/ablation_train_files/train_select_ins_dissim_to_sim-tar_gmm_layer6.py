...
from models.yolo_feature_layer6 import Model_feature  # 为了返回最后的特征 额外定义的特征
...
from utils.gmm_index import get_index_via_gmm # get_index_via_gmm(source_ins, target_ins)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if targets_t_.shape[0] > 0:
                feature_t = get_ins_feature(feature_img_t, targets_t_, device, number=50).to(device)
            else:
            ...
            with amp.autocast(enabled=cuda):
                ...
                if feature_t.shape[0] >= 20 and feature_s.shape[0] >= 50:  
                    #k = int(feature_s.shape[0]*opt.k_por) # 0.5 is the ratio for interested tar_sim
                    #tar_sim_idx = MMD_distance(feature_t, feature_s, k)
                    #tar_dissim_idx = torch.tensor([i for i in range(feature_s.shape[0]) if i not in tar_sim_idx])
                    tar_dissim_idx, tar_sim_idx = get_index_via_gmm(feature_s, feature_t)
                    tar_sim = feature_s[tar_sim_idx, :]
                    tar_dissim = feature_s[tar_dissim_idx, :]
                else:
                    tar_sim = feature_t.clone()
