...
from models.yolo_feature_layer4 import Model_feature  # 为了返回最后的特征 额外定义的特征
...
from utils.loss_mul import compute_loss
...
def train(hyp, opt, device, tb_writer=None, wandb=None):  # opt.data 是数据集加载的yaml文件信息
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if targets_t_.shape[0] > 0:
                feature_ins_t = get_ins_feature(feature_img_t, targets_t_, device, number=80).to(device)
            else:
                # replace ins-level to img-level for complementing the representation
                feature_ins_t = feature_img_t.mean(3).mean(2)
            feature_img_t_avg = feature_img_t.mean(3).mean(2)
            ...
            with amp.autocast(enabled=cuda):
                ...
                if targets.shape[0] > 0:
                    feature_ins_s = get_ins_feature(feature_img_s, targets, device, number=300).to(device)
                else:
                # replace ins-level to img-level for complementing the representation
                    feature_ins_s = feature_img_s.mean(3).mean(2)
                feature_img_s_avg = feature_img_s.mean(3).mean(2)
                
                loss, loss_items = compute_loss(pred, targets.to(device), model, feature_ins_s, feature_ins_t, feature_img_s_avg, feature_img_t_avg)  # loss scaled by batch_size
                if rank != -1:
