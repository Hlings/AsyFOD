...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)): # 对每一个batch推断
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            inf_out, train_out = model(img, augment=augment)[0]  # inference and training outputs
    ...
    if len(stats) and stats[0].any():
        ...
        print("ap list is") # 尝试打印出不同iou取值下的性能指标
        print(ap)
    ...
    if save_json and len(jdict):
        ...
        anno_json = '/userhome/COCO/annotations/instances_val2017.json'  # annotations json
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conf-thres', type=float, default=0.000, help='object confidence threshold')
