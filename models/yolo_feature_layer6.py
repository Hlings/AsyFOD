    ...
    def __init__(self, in_dim=640):
        ...
        self.ranker = nn.Sequential(  # input [B, C, H, W] output [B, 3, H, W]
            ...
            nn.Conv2d(
                ...
                out_channels=self.in_dim, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(self.in_dim),
            )
...
class Model_feature(nn.Module):
    ...
    def forward(self, x, augment=False, profile=False):
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                yi = self.forward_once(xi)[0]  # forward  这行需要改 当前当augment为True的时候应该会报错
    ...
    def forward_once(self, x, profile=False): # x input 此为模型进行前向传播的过程
        y, dt = [], []  # outputs
        cur_layer = 0   # mark cur_layer num if 9 detach 
        for m in self.model:
            ...
            x = m(x)  # run  进行前向传播的过程
            if cur_layer == 6:
                backbone_feature = x # output backbone feature  output [B, 1280, 14, 21]
        ...
        return x, self.ranker(backbone_feature)
