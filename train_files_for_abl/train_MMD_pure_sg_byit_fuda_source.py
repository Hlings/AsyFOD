...
def train(hyp, opt, device, tb_writer=None, wandb=None):  # opt.data 是数据集加载的yaml文件信息
    ...
    bs_source = batch_size   # source dataloader built by bs but choose topk for training
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar: # pbar [(0, (2, 3)), (1, (2, 3))]
            ...
            imgs = torch.cat((imgs, imgs_tar_), 0) # refine
            targets = torch.cat((targets, targets_tar_), 0) #refine
            paths = paths + paths_tar
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                feature_s = feature_s.mean(3).mean(2).detach()
