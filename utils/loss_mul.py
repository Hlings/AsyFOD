...
# predictions, targets, model, feature of source batch, feature of target batch
def compute_loss(p, targets, model, feature_s=None, feature_t=None, feature_s_img=None, feature_t_img=None):
    ...
    if feature_s is not None:
        ...
        lmmd = 0.5*(mmd(feature_s, feature_t).to(device)+mmd(feature_s_img, feature_t_img).to(device))
    ...
    lcls *= h['cls'] * s
    lmmd *= 1 # default as 1 now trying 2 3 for ablation stuides
