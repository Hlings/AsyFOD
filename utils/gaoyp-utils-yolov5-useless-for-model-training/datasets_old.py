...
def load_mosaic(self, index):
    ...
    # copy-paste
    img4, labels4 = copy_paste(img4, labels4)
    # Augment
...
def copy_paste(im, labels, p=0.5): # im是一个三维的图像 单张img
    """
    ...
    for i in range(change_times-1):   
        cp_labels = random.sample(list(labels),2) #初步demo 只选定两个label 进行交互
        im = change_target(im, cp_labels) # 将两个labels的内容进行交换
    
    return im, labels

def change_target(im,cp_labels): # cp_labels 为含有两个元素的labels列表
    ...
    im[y1a:y1b, x1a:x1b] = target2
    im[y2a:y2b, x2a:x2b] = target1
