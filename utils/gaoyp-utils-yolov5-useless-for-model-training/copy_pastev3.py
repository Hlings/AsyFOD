...
def copy_paste(im, labels, cp_type, p=0.9): # im是一个三维的图像 单张img
    """
    ...
    for cls_number in cls_unique:
        ...
        for times in range(int(m*p)+6):   
            cp_labels = random.sample(labels_refine.tolist(),2) #初步demo 只选定两个label 进行交互
            im = change_target(im, cp_labels, labels_refine, cp_type) # 将两个labels的内容进行交换
    
    return im, labels


def change_target(im, cp_labels, labels, cp_type, ioa_th=0.3, margin=0.5): # cp_labels 为含有两个元素的labels列表
    #_, w, _ = im.shape
    ...
    labels_moved_1, labels_moved_2 = labels.tolist().copy(), labels.tolist().copy()  # 计算ioa的时候避免和自己进行计算
