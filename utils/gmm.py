...
class GaussianMixture(torch.nn.Module):
    ...
    """
    def __init__(self, n_components, n_features, covariance_type="diag", eps=1.e-6, init_params="kmeans", mu_init=None, var_init=None):
        """
    ...
    def predict(self, x, probs=False, ood=False):
        """
        Assigns input data to one of the mixture components by evaluating the likelihood under each.
        If probs=True returns normalized probabilities of class membership.
        IF ood=True returns simple probabilty to gmm (applied in out-of-distribution estimation)
        args:
            x:          torch.Tensor (n, d) or (n, 1, d)
            probs:      bool
            ood:        bool 
