...
def copy_paste_api(imgs_source, imgs_target, labels_source,
                   labels_target):  # imgs_source shape () labels_source: xywh格式
...
def copy_paste(im1, im2, labels1, labels2, p=0.6):  # im是一个三维的图像 单张img im shaoe [H, W, C]
    """
    ...
    # print("n is", n)
    for cls_number in cls_unique:  # 这里对每一个类别进行筛选
        ...
        for times in range(int(m1 * p) + 3):
            cp_labels = random.sample(labels1_refine.tolist(), 1) + random.sample(labels2_refine.tolist(), 1)  # 初步demo 只选定两个label 进行交互                                   
            #im1, im2 = change_target(im1, im2, cp_labels, labels1_refine, labels2_refine)  # 将两个labels的内容进行交换
            im1, im2 = change_target(im1, im2, cp_labels, labels1_refine, labels2_refine)  # 将两个labels的内容进行交换
            
    return im1, im2, labels1, labels2

"""
def change_target(im1, im2, cp_labels, gaussian=False):  # cp_labels 为含有两个元素的labels列表
...
def change_target(im1, im2, cp_labels, labels1_refine, labels2_refine, ioa_th=0.4, margin=0.6,
                  gaussian=False):  # cp_labels 为含有两个元素的labels列表
    # _, w, _ = im.shape
...
def _gaussian_map(height, width, x1, y1, x2, y2):
    ...
    mean_torch = torch.tensor([h // 2, w // 2])  # bbx的中心点

    # r_var = (height * width / (2 * math.pi)) ** 0.5  # * 0.8  #全局影响因子？
